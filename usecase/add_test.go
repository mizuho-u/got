...
func TestAddSingleFile(t *testing.T) {
	...
	// act
	if err := usecase.Add(dir, f); err != nil {
		t.Fatal(err)
	}
...
func TestAddMultipleFiles(t *testing.T) {
	...
	// act
	if err := usecase.Add(dir, f1, f2); err != nil {
		t.Fatal(err)
	}
...
func TestAddFilesFromDirectory(t *testing.T) {
	...
	// act
	if err := usecase.Add(dir, dir); err != nil {
		t.Fatal(err)
	}
...
func TestModifyTheIndex(t *testing.T) {
	...
	// act
	if err := usecase.Add(dir, f1); err != nil {
		t.Fatal(err)
	}
	if err := usecase.Add(dir, f2); err != nil {
		t.Fatal(err)
	}
